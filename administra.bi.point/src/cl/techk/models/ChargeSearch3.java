...
public class ChargeSearch3 {
    ...
    public static JSONObject charge(String upload_file_directory, int market_id) {
        ...
        try {
            Utils.print("charging search3");
            StreamingReader excel_reader = ExcelUtils.getFileStreamingReader(upload_file_directory, "B3");
    ...
    public static JSONObject getReadedExcelRow(String market_name, Row xlsx_row) {
        ...
        for (int index_column = 0; index_column < ValidateSearch3.COLUMN_COUNT; index_column++) {
            
            Cell xlsx_cell = xlsx_row.getCell(index_column);

            if (!ValidateSearch3.validateCellData(index_column, row_index, market_name, xlsx_cell, array_row_data,
                    errors_on_data)) {
