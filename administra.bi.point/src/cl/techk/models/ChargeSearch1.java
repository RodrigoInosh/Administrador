...
public class ChargeSearch1 {
    ...
    public static JSONObject charge(String upload_file_directory, int market_id) {
        ...
        try {
            Utils.print("charging search1");
            StreamingReader excel_reader = ExcelUtils.getFileStreamingReader(upload_file_directory, "B1");
    ...
    public static JSONObject getReadedExcelRow(String market_name, Row xlsx_row) {
        ...
        for (int index_column = 0; index_column < ValidateSearch1.COLUMN_COUNT; index_column++) {
            try {
            Cell xlsx_cell = xlsx_row.getCell(index_column);
            
            if (!ValidateSearch1.validateCellData(index_column, row_index, market_name, xlsx_cell, array_row_data,
                    errors_on_data)) {
