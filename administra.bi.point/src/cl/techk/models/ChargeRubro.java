...
public class ChargeRubro {
    ...
    public static JSONObject charge(String upload_file_directory, int market_id) {
        ...
        try {
            Utils.print("charging rubros");
            StreamingReader excel_reader = ExcelUtils.getFileStreamingReader(upload_file_directory, "Rubros");
    ...
    public static JSONObject getReadedExcelRow(String market_name, Row xlsx_row) {
        ...
        for (int index_column = 0; index_column < ValidateRubro.COLUMN_COUNT; index_column++) {
            
            Cell xlsx_cell = xlsx_row.getCell(index_column);
            
            if (!ValidateRubro.validateCellData(index_column, row_index, market_name, xlsx_cell, array_row_data,
                    errors_on_data)) {
